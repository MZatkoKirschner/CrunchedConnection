...
# 2. For now, download different forecast periods but only use 24 hour forecast in ML model
...
callsignsFile = '../flights/callsigns_unique.txt'
...
callsignsMore = ['KACK','KAKN','KATY','KBFM','KBKG','KDLG','KEFD','KENV','KFLO',
    'KFNL','KGST','KHYA','KMVY','KPAE','KPGV','KPIR','KWYS','KXWA']
...
for year in years:
    ...
    while date <= enddate:
        ...
        for i in range(0,len(callsignsMore)):
            ...
            #Drop extra columns
            #rawMettmp.drop(['obv','poz','pos','typ','sky','swh','lcb','i06','slv',
            #    's06','pra','ppl','psn','pzr','t03','gst'],inplace=True,axis=1)
    ...
    #met.to_hdf(str(year)+'_met.h5', key='met', mode='w')
